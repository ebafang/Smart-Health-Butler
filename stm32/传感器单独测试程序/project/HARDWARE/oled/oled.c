...
void OLED_ShowChinese(uint8_t x,uint8_t y,uint8_t num,uint8_t size1,uint8_t mode)
{
	...
	{
		if(size1 == 10)
		{temp=Hzk10[num][i];}//调用10*10字体
		else if(size1 == 12)
				{temp=Hzk12[num][i];}//调用12*12字体
		else if(size1==16)
...
/*
	...
	len： 汉字的个数
*/
void OLED_Show_Line_Chinese(uint8_t x,uint8_t y,uint8_t num,uint8_t size1,uint8_t len)
{
	uint8_t i;
	for(i = 0;i<len;i++)
	{
		OLED_ShowChinese(x+i*size1,y,num+i,size1,1);
	}
}
