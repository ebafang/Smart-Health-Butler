...
// 非阻塞检测触摸屏是否被按下（任意位置）
static int ts_is_pressed(void)
{
	int ts = open("/dev/input/event0", O_RDONLY | O_NONBLOCK);
	if (ts < 0) return 0;

	struct input_event ev;
	while (read(ts, &ev, sizeof(ev)) > 0)
	{
...
xmlChar *wait4id(int sockfd)
{
	...
	while (timeout > 0)
	{
		...
		if (ret < 0)
		{
			perror("select() in wait4id");
			break;
		}
		else if (ret > 0)
		{
			// socket有数据可读
			int n = read(sockfd, xml, XMLSIZE - 1);
			if (n > 0)
			{
				total_read = n;
		...
		if (ts_is_pressed())
		{
			printf("Touch cancel detected, aborting wait4id.\n");
			free(xml);
			return NULL;
		}
	...
	size_t m = fwrite(xml, 1, total_read, fp);
	fclose(fp);
	free(xml);
